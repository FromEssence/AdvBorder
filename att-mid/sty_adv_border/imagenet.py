...
class ImageNet:
    ...
    @staticmethod
    def get_bordering(width, img_id, keep_size=True):
        return bordering(width=width, image_side_length=ImageNet.SIDE_LENGTH, normalize=ImageNet.normalize_not_in_place,img_id=img_id,
                       scale=1., keep_size=keep_size)

    @staticmethod
    def get_data_loaders(data_path, batch_size, num_workers, normalize=True, shuffle_val=False):
...
def load_pretrained_imagenet_bordering(checkpoint_path, imgid = 1):
    checkpoint = torch.load(checkpoint_path, map_location=lambda storage, loc: storage)
    args = checkpoint['args']
    bordering = ImageNet.get_bordering(args.width, imgid, keep_size=True)#getattr(args, 'keep_size', False))
    bordering.load_state_dict(checkpoint['bordering'])
    bordering.eval()
    return bordering
