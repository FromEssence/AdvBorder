# -*- CODING: Utf-8 -*-
# expand targetd attack set attack weight=1000
import os
...
from imagenet import ImageNet, load_pretrained_imagenet_bordering
...
SUCCESS = torch.tensor(100, device=device)
...
class Trainer:
    def __init__(self, args):
        ...
        self.border_chk_save_path = args.border_chk_save_path
        self.border_png_save_path = args.border_png_save_path
    ...
    def process_epoch_lbfgs(self, cnn2, normalization_mean, normalization_std,
        ...
        imgnames.sort()
        targets = []
        with open('../dataset/targets.txt', 'rt') as f:
            lines = f.readlines()
            for line in lines:
                targets.append(int(line.strip('\n')))
        for i, (input, target) in enumerate(data_loader):
            if i<(int)(self.start_img_index)*1000:
                continue
            if i>=1000+(int)(self.start_img_index)*1000:
                break
            ...
            # 目标攻击
            choose_t = targets[i]
            #while choose_t == (int)(imgnames[i][:8]):
             #   choose_t = random.randint(0,999) #(int)(imgnames[i][:8])
            self.target = choose_t
            ...
            self.bordering = self.dataset.get_bordering(self.args.width, img_id=i, keep_size=True).to(self.device) #随机初始化
            self.optimizer_att_lbgfs = torch.optim.LBFGS([self.bordering.attack], lr=1.5, max_iter=10) 
            ...
            def closure():
                ...
                opt_img = torch.zeros((1,3,32,220)).to(self.device)
                # 计算风格损失
                opt_img[0] = self.bordering.attack[:,:,:,0]
                ...
                with torch.set_grad_enabled(train):
                    input_att, _ = self.bordering(input=self.img_input) # 添加边框
            ...
            while j<600:
                ...
                if j%10 == 0:
                    ...
                    for param_group in self.optimizer_att_lbgfs.param_groups:
                        param_group['lr'] *= self.args.lr_decay_coefficient
                if flag == True and cnt_succ>50:
                    break
            opt_img = torch.ones((1,3,32,220))
            opt_img[0] = self.bordering.attack[:,:,:, 0]

            saved_img = postp(opt_img.data[0].cpu().squeeze())
            plt.imsave(self.border_png_save_path+imgnames[i], np.array(saved_img))
            print("final acc,loss_sty,loss_att:", final_acc, final_loss_sty, final_loss_att)
            torch.save({
                    
                    'bordering': self.bordering.state_dict(),
                    #'optimizer': self.optimizer.state_dict(),
                    'args': self.args,
            }, self.border_chk_save_path+'0'*(5-len(str(i)))+str(i)+'.chk')
