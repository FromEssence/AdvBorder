...
if __name__ == '__main__':
    ...
    for i, (input, target) in enumerate(data_loader):
        ...
        bordering = load_pretrained_imagenet_bordering(pretrained_path+chk_name[i]).to(device)
        with torch.no_grad():
            input_att, _ = bordering(input, do_normalize=False)
        ...
        cnt_total += 1
        print(i, output_att[0].item())#, output_att[i].item())#
        if args.defense==0:
